...
**用途：** Claude Code CLI 完整命令参考。日常使用请参考 [交互与对话](../02-interaction/) 章节。

# CLI 命令参考手册

> **注意：** 这是一个参考手册，包含所有 CLI 命令和标志。如果你想学习如何使用 CLI，请先阅读 [交互与对话](../02-interaction/) 章节。
...
| 模型 | 适用场景 | 速度 | 成本 |
|------|---------|------|------|
| **Opus 4.6** | 复杂任务、深度分析、架构设计 | 中等 | 最高 |
| **Sonnet 4.6** | 一般编程、文档生成、代理任务 | 快速 | 中等 |
| **Haiku 4.5** | 快速查询、简单任务、批量处理 | 最快 | 最低 |
| **opusplan** | Opus 规划 + Sonnet 执行 | 中等 | 中等 |
...
| `acceptEdits` | 自动接受编辑操作 | 代码修改场景 |
| `plan` | 只读规划模式 | 安全审查、架构分析 |
...
# 允许特定命令模式
--allowedTools "Bash(git log:*)" "Bash(npm test:*)"
...
| `--input-format` | 指定输入格式（print 模式） | `text`、`json`、`yaml`、`markdown` | `claude -p --input-format json` |
...
# JSON 输入 + JSON 输出（格式转换）
cat config.yaml | claude --input-format yaml --output-format json -p "转换为JSON"
```

### 流式输出处理

使用 `--include-partial-messages` 实时处理流式输出：

```bash
# 实时监控流式输出
claude --include-partial-messages --output-format stream-json -p "生成文档" | while read line; do
  echo "收到: $line"
done
...
# 添加共享库访问
claude --add-dir ../shared-lib "使用共享库函数"

# 添加测试目录
claude --add-dir ./tests "分析测试覆盖率"
```
